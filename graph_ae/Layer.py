...
from torch_geometric.nn import global_add_pool as g_pooling, GATConv, GCNConv
...
class MultiGCNConv(torch.nn.Module, ABC):
    """Multi-kernel GCNConv"""
    def __init__(self, size, in_channel, out_channel, heads: int = 1):
        super(MultiGCNConv, self).__init__()
        self.size = size
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.heads = heads
        ...
        for i in range(size):
            self.gat_list.append(GCNConv(in_channel, out_channel))   # .to(device)
...
class MultiGATConv(torch.nn.Module, ABC):
    """Multi-kernel GAT Conv"""

    def __init__(self, size, in_channel, out_channel, heads: int = 1):
        super(MultiGATConv, self).__init__()
        self.size = size
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.heads = heads
