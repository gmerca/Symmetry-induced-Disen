...
class Modularity:
    def __init__(self, ds, num_points=5000, paired=False, device="cpu"):
    ...
    def __call__(self, model):
        scores = {}
        rep_fn = lambda x: model(x.to(self.device))
        ...
        scores["dmetric/modularity_score"] = "{:.4f}".format(modularity(mutual_information))
        ...
        scores["dmetric/explicitness_score_train"] = "{:.4f}".format(np.mean(explicitness_score_train))
        scores["dmetric/explicitness_score_test"] = "{:.4f}".format(np.mean(explicitness_score_test))
        return np.mean(explicitness_score_test)
