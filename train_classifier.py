...
from utils.train_utils import load_model_result, train_cf, select_model, get_pyg_dataset, run_demo, setup
...
def main(rank, world_size, arg):
    ...
    dist.barrier()
    map_loc = {'cuda:%d' % 0: 'cuda:%d' % rank} if torch.cuda.is_available() else None
    model.load_state_dict(torch.load(arg.model_dir + arg.m + f"_{arg.d}" + ".ckpt", map_location=map_loc), strict=True)
    group1, group2 = load_model_result(model, train_set, test_set, rank)
    ...
    train_cf(c_model, optimizer, rank, train_set, test_set, num_epoch, group1, group2, arg.model_dir, arg.m, arg.d)
...
if __name__ == "__main__":
    ...
    # lie group VAE
    parser.add_argument('--group', type=int, default=81, help="group size")
    ...
    parser.add_argument('--cond', action='store_true', default=False, help='conditional disen')
